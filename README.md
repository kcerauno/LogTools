...
細かい方から粗い方へ、以下の9段階を用意しています(1回の実行で
指定できるのは1段階のみ)。
...
| `daynight` | day/nightの2区分に丸める(下記参照) | `20091209 Wed day` |
...
1日を昼夜2区分に分けて集計するレベルです。デフォルトでは
7時〜22時を`day`、22時〜7時を`night`として丸めます。nightは
日をまたぐ範囲になるため、日付は「nightが開始した時点の日付」を
使います(0時〜day開始時刻未満のタイムスタンプは、前日のnightに
含めます)。

```
20091209 Wed 14:55:10 → 20091209 Wed day
20091209 Wed 22:55:10 → 20091209 Wed night
20091210 Thu 01:55:10 → 20091209 Wed night   # 0時をまたいでも前日のnight扱い
```

`--day-start-hour` / `--night-start-hour` (共に0〜23の整数、
`--day-start-hour` は `--night-start-hour` より小さい値)で、
day/nightの境界時刻を変更できます。`--level daynight` 以外を
指定した場合、これらのオプションは無視されます。
