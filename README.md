...
- [`logts-tag`](#logts-tag) — ログファイルの日時フォーマットを識別し、変換後タイムスタンプを各行に付与する
...
インストールすると `logts-tag` コマンドが使えるようになります。
...
```bash
logts-tag access1.log access2.log
```

標準入力からも読めます(パイプ処理向け)。

```bash
tail -f app.log | logts-tag
...
```bash
# 出力フォーマットを変える
logts-tag --output-format '%Y-%m-%d %H:%M:%S' app.log

# 2024年のアーカイブログ(syslog形式で年が書かれていない)を処理する場合
logts-tag --default-year 2024 archive/syslog
```
...
ものを採用します(実体は
[`src/wk_logtool/logts_tag/patterns.py`](src/wk_logtool/logts_tag/patterns.py))。
